...
create_bundle profile TargetDir:
    ...
    if [ "{{profile}}" == "release" ]; then
        ...
        cp "{{TargetDir}}/x86_64-apple-darwin/release/{{BinaryName}}.rsrc" "{{TargetDir}}/{{profile}}/{{PluginName}}.plugin/Contents/Resources/{{BuildName}}.rsrc"
        lipo "{{TargetDir}}/{x86_64,aarch64}-apple-darwin/release/lib{{BinaryName}}.dylib" -create -output "{{TargetDir}}/{{profile}}/{{PluginName}}.plugin/Contents/MacOS/{{BuildName}}.dylib"
        mv "{{TargetDir}}/{{profile}}/{{PluginName}}.plugin/Contents/MacOS/{{BuildName}}.dylib" "{{TargetDir}}/{{profile}}/{{PluginName}}.plugin/Contents/MacOS/{{BuildName}}"
    else
        cp "{{TargetDir}}/{{profile}}/{{BuildName}}.rsrc" "{{TargetDir}}/{{profile}}/{{PluginName}}.plugin/Contents/Resources/{{BuildName}}.rsrc"
        cp "{{TargetDir}}/{{profile}}/lib{{BinaryName}}.dylib" "{{TargetDir}}/{{profile}}/{{PluginName}}.plugin/Contents/MacOS/{{BuildName}}"
    fi
